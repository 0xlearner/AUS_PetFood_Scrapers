...
class BestFriendsPets:
    ...
    def to_csv(self, results):
        ...
        csv_file = "bestfriendspets_products.csv"
        if not os.path.isfile(csv_file):
            df.to_csv(csv_file, index=False)
        else:
            df.to_csv(csv_file, mode="a", encoding="utf-8", header=False, index=False)

        print('Stored results to "bestfriendspets_products.csv"')

    def run(self):
        filters = ["185088311432", "189656072328"]
        for fi in filters:
            self.params["bgfilter.collection_id"] = fi
            url = "https://api.searchspring.net/api/search/search.json"
